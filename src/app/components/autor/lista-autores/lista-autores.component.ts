...
export class ListaAutoresComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  title: string = 'Lista de Autores';

  @ViewChild(DataTableDirective) dtElement: DataTableDirective;
  dtInstance: Promise<DataTables.Api>;
  dtTrigger: Subject<any> = new Subject();

  constructor(private autorService:AutorService, private router:Router, private http: HttpClient) { }
  ...
  private obtenerAutores(){
    this.autorService.obtenerListaDeAutores().subscribe(dato => {
      this.autores = dato;
	    this.loadData = true;
      ...
      this.dtOptions = {
        ...
        drawCallback: () => {
          ...
          $('#datatables tbody').on('click', '.eliminar', function () {
            ...
            const autor = self.autores.filter(element => element.id == this.id);
  ...
  actualizarAutor(id:number){
    ...
    this.router.navigate(['actualizar-autor',id]);
  }

  eliminarAutor(id:number){
         
    this.autorService.eliminarAutor(id).subscribe(dato => {
        // console.log(dato);
       
        // busca el `autor` por `id` dentro del arreglo `autores`
        const autor = this.autores.filter(element => element.id == id);
        ...
        let self = this
        let content = document.createElement('div');
        content.innerHTML = 'El autor: `<strong>'+ autor[0].autor +'</strong>`, ha sido  <strong>eliminado</strong> con exito!!!';
  
        Swal.fire({ title: 'Eliminar Autor',
                     html: content,
                     icon: `success`})
            .then(function () {                
  ...
  verDetalles(id:number){
    ...
    this.router.navigate(['autor-detalles',id]);   
  }
  ...
  refreshData(){
    ...
    this.autorService.obtenerListaDeAutores().subscribe(dato => {
      this.autores = dato;
      this.dtOptions.data = this.autores;
	    this.loadData = true;
    });
  }
}
