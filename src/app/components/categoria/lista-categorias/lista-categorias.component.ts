...
export class ListaCategoriasComponent implements OnInit {
  ...
  private obtenerAutores(){
    this.categoriaService.obtenerListaDeCategorias().subscribe(dato => {
      ...
      this.dtOptions = {
        ...
        columns: [{ data: 'id' }, { data: 'categoria' },
        ...
        drawCallback: () => {
          ...
          $('#datatables tbody').on('click', '.eliminar', function () {
            ...
            const categoria = self.categorias.filter(element => element.id == this.id);
          
            let content = document.createElement('div');
            content.innerHTML = 'Deseas realmente eliminar la categoría: <strong>' + categoria[0].categoria + '</strong> ?';
            ...
            Swal.fire({
              ...
              if ( result.isConfirmed ) {
                self.eliminarCategoria(this.id);          
  ...
  eliminarCategoria(id:number){
    ...
    this.categoriaService.eliminarCategoria(id).subscribe(dato => {
        ...
        const categoria = this.categorias.filter(element => element.id == id);
                          
        // Swal.fire('Eliminar Categoría',`La categoría: ${categoria[0].categoria}, ha sido eliminada con exito!!!`,`success`);              

        let self = this
        let content = document.createElement('div');
        content.innerHTML = 'La categoría: `<strong>'+ categoria[0].categoria +'</strong>`, ha sido  <strong>eliminada</strong> con exito!!!';
  
        Swal.fire({ title: 'Eliminar Categoría',
                     html: content,
                     icon: `success`})
            .then(function () {                
                  self.router.navigate(['/categorias']);
                 })
  ...
  refreshData(){
    ...
    this.categoriaService.obtenerListaDeCategorias().subscribe(dato => {
      this.categorias = dato;
      this.dtOptions.data = this.categorias;
	    this.loadData = true;
    });
  }
